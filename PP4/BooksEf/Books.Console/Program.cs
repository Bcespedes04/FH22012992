...
static async Task LoadFromCsvAsync(BooksContext db)
{
    ...
    {
        Console.WriteLine("❌ No se encontró el archivo CSV en ./data/books.csv");
        return;
    }

    var lines = await File.ReadAllLinesAsync(csvPath);

    // primera línea es el encabezado: Author,Title,Tags
    foreach (var line in lines.Skip(1))
    {
        ...
        // caso con autor entre comillas: "Apellido, Nombre",Titulo,Tags
        if (line.StartsWith("\""))
        {
        ...
        // 1️⃣ autor (si no existe, lo creamos)
...
static async Task ExportToTsvAsync(BooksContext db)
{
    ...
    foreach (var g in groups)
    {
        ...
        // orden descendente Autor > Título > Tag
        var ordered = g
